...
enum StaticFileHandlerError : Error {
...
public struct StockStaticFileHandler {
  ...
  public func handleRequest(request: HTTPRequest, response: HTTPResponse) {
    ...
    let filePath = documentRoot + request.path
    do {
      // defend directory traversal attack
      let realPath = try POSIX.realpath(filePath)
      if realPath != filePath {
        throw StaticFileHandlerError.InvalidPath(request.path)
      }
    } catch {
      return fnf(msg: "The file \(request.path) was not found.")
    }
    ...
    do {
      ...
      return fnf(msg: "The file \(request.path) could not be opened \(error).")
    }
  }
